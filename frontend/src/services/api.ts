...
export const taiKhoanApi = {
  ...
  capNhatThongTin: (data: { hoTen: string; soDienThoai?: string; email?: string; gioiTinh?: boolean; ngaySinh?: string }) =>
    api.put<ApiResponse<Pick<User, 'id' | 'hoTen' | 'email' | 'soDienThoai'>>>('/tai-khoan/thong-tin', data),
...
export const adminApi = {
  ...
  sanPhamSapHetHang: () =>
    api.get<ApiResponse<TonKhoCanhBao[]>>('/quan-ly/thong-ke/san-pham-sap-het-hang'),

  sanPhamHetHang: () =>
    api.get<ApiResponse<TonKhoCanhBao[]>>('/quan-ly/thong-ke/san-pham-het-hang'),
  ...
  taoNhanVien: (data: { tenDangNhap: string; matKhau: string; hoTen: string; email: string; soDienThoai?: string }) =>
...
export const banHangApi = {
  danhSachCho: () =>
      api.get<DonHang[]>('/quan-ly/ban-hang/danh-sach-cho'),

  taoHoaDon: () =>
      api.post<{ thanhCong: boolean; idHoaDon: number; maHoaDon: string }>('/quan-ly/ban-hang/tao-hoa-don'),

  chiTietHoaDon: (id: number) =>
      api.get<{ chiTiet: unknown[]; tongTien: number; thanhCong: boolean; id: number; maHoaDon: string }>  (`/quan-ly/ban-hang/${id}`),

  timSanPham: (q: string) =>
      api.get<unknown[]>('/quan-ly/ban-hang/tim-san-pham', { params: { q } }),

  themSanPham: (hoaDonId: number, bienTheId: number, soLuong: number) =>
      api.post<ApiResponse<null>>(`/quan-ly/ban-hang/${hoaDonId}/them-san-pham`, null, { params: { bienTheId, soLuong } }),

  capNhatSoLuong: (chiTietId: number, soLuong: number) =>
      api.post<ApiResponse<null>>(`/quan-ly/ban-hang/chi-tiet/${chiTietId}/cap-nhat`, null, { params: { soLuong } }),

  xoaSanPham: (chiTietId: number) =>
      api.post<ApiResponse<null>>(`/quan-ly/ban-hang/chi-tiet/${chiTietId}/xoa`),

  apVoucher: (hoaDonId: number, maVoucher: string) =>
      api.post<{ thanhCong: boolean; soTienGiam: number; tongSauGiam: number; thongBao: string }>(`/quan-ly/ban-hang/${hoaDonId}/ap-voucher`, null, { params: { maVoucher } }),

  xoaVoucher: (hoaDonId: number) =>
      api.post<ApiResponse<null>>(`/quan-ly/ban-hang/${hoaDonId}/xoa-voucher`),

  thanhToan: (hoaDonId: number, data: { phuongThucThanhToan: string; tienKhachDua?: number; tenKhach?: string; sdtKhach?: string }) =>
      api.post<ApiResponse<null>>(`/quan-ly/ban-hang/${hoaDonId}/thanh-toan`, null, { params: data }),

  huyHoaDon: (hoaDonId: number, lyDo?: string) =>
      api.post<ApiResponse<null>>(`/quan-ly/ban-hang/${hoaDonId}/huy`, null, { params: { lyDo } }),
};
